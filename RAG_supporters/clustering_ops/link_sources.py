...
class SourceClusterLinker:
    ...
    def __init__(self, cluster_parser: ClusterParser, fallback_strategy: str = "largest"):
    ...
    def link_pair(self, keywords: List[str], pair_id: Optional[int] = None) -> int:
        ...
        if not matched_clusters:
            LOGGER.debug(
                f"Pair {pair_id} keywords {keywords} did not match any cluster, " f"using fallback"
            )
            return self._get_fallback_cluster(pair_id)
    ...
    def link_batch(
        self, keywords_list: List[List[str]], pair_ids: Optional[List[int]] = None
    ) -> List[int]:
    ...
    def link_dataframe(
        ...
        if pair_id_col not in df.columns:
            LOGGER.warning(f"Pair ID column '{pair_id_col}' not found, using DataFrame index")
        ...
        if show_progress:
            iterator = tqdm(iterator, total=len(df), desc="Linking pairs to clusters")
    ...
    def validate_assignments(
        self, cluster_assignments: Union[List[int], np.ndarray, torch.Tensor]
    ) -> Dict[str, any]:
        """Validate cluster assignments for dataset integrity.
        ...
        if max_id >= self.n_clusters:
            errors.append(f"Invalid cluster ID found: {max_id} " f"(must be < {self.n_clusters})")
...
def link_sources(
    ...
    """
    linker = SourceClusterLinker(cluster_parser=cluster_parser, fallback_strategy=fallback_strategy)
