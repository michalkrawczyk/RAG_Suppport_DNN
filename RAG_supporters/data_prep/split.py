...
class DatasetSplitter:
    ...
    def __init__(
        ...
        # Validate inputs
        self._validate_inputs(pair_indices, pair_cluster_ids, train_ratio, val_ratio, test_ratio)
    ...
    def _build_question_to_pairs(self) -> Dict[int, List[int]]:
        ...
        LOGGER.info(f"Built question-to-pairs mapping: {len(question_to_pairs)} questions")
    ...
    def _assign_question_clusters(self, question_to_pairs: Dict[int, List[int]]) -> Dict[int, int]:
        ...
        if self.show_progress:
            iterator = tqdm(
                iterator, total=len(question_to_pairs), desc="Assigning question clusters"
            )
        ...
        LOGGER.info(f"Assigned {len(question_clusters)} questions to primary clusters")
    ...
    def split(self) -> Dict[str, torch.Tensor]:
        ...
        # Perform stratified split at question level
        train_questions, val_questions, test_questions = self._stratified_split_questions(
            question_clusters
        )
    ...
    def _validate_splits(
        self, train_idx: torch.Tensor, val_idx: torch.Tensor, test_idx: torch.Tensor
    ) -> None:
        ...
        if overlap_train_val:
            raise ValueError(f"Train and val splits overlap: {len(overlap_train_val)} pairs")

        if overlap_train_test:
            raise ValueError(f"Train and test splits overlap: {len(overlap_train_test)} pairs")

        if overlap_val_test:
            raise ValueError(f"Val and test splits overlap: {len(overlap_val_test)} pairs")
