...
class TorchDatasetRAG(Dataset):
    ...
    def __init__(self, base_dataset: BaseDatasetRAG, transform=None,
        ...
        if use_contrastive_samples:
            self.sample_holders.extend([s for s in self.base_dataset.load_samples_pairs("contrastive") if s.get('label')])
        if use_positive_samples:
            self.sample_holders.extend([s for s in self.base_dataset.load_samples_pairs("positive") if s.get('label')])
        if use_similar_samples:
            self.sample_holders.extend([s for s in self.base_dataset.load_samples_pairs("similar") if s.get('label')])
...
class TorchDataLoaderRAG(DataLoader):
    def __init__(self, dataset: TorchDatasetRAG, batch_size: int, shuffle: bool, num_workers: int):
        super(TorchDataLoaderRAG, self).__init__(dataset, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers)
