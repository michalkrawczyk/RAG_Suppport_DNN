...
from prompts_templates.rag_verifiers import FINAL_VERDICT_PROMPT, SRC_COMPARE_PROMPT_WITH_SCORES, create_verifying_chain
...
class SampleTripletRAGChroma:
    ...
    answer_id_1: str
    answer_id_2: str
    label: int = -1 # 1 if answer_1 is better, 0 if answer_2 is better, -1 if not labeled
...
class BaseRAGDatasetGenerator(ABC):
    ...
    @abstractmethod
    def _generate_positive_triplet_samples(self, question_chroma_id, relevant_passage_ids, **kwargs) \
    ...
    @abstractmethod
    def _generate_contrastive_triplet_samples(self, question_chroma_id, relevant_passage_ids,
                                              num_negative_samples: int = 2, keep_same_negatives=False, **kwargs) \
            -> List[SampleTripletRAGChroma]:
        """
    ...
    @abstractmethod
    def _generate_similar_triplet_samples(self, question_chroma_id, relevant_passage_ids, **kwargs) \
            -> List[SampleTripletRAGChroma]:
        """
    ...
    def _samples_triplet_generator(self, sample_type: str,
        ...
        for question_id in tqdm(db_data['ids'], desc=f"Generating {sample_type} samples"):
            question_embedding = db_data['embeddings'][question_id]
            relevant_passage_ids = db_data['metadatas'][question_id][relevant_ids_field_name]

            sample_triplets.append(generator_func(question_embedding, relevant_passage_ids, **kwargs))
    ...
    def chroma_id_to_embedding(self, chroma_ids: Union[List[str], str], search_db: str):
        ...
        return db_to_search.get(ids=chroma_ids if isinstance(chroma_ids, list) else [chroma_ids],
                                include=["embeddings"])['embeddings']


    def get_question_db_data(self, include: Iterable[str] = ("documents", "metadatas", "embeddings")):
        return self._question_db.get(include=list(include))

    def validate_triplet_samples(self, llm, samples: List[SampleTripletRAGChroma],
                                 analysis_prompt: str = SRC_COMPARE_PROMPT_WITH_SCORES,
                                 answer_extraction_prompt:str = FINAL_VERDICT_PROMPT,
                                 skip_labeled: bool = True) -> List[SampleTripletRAGChroma]:

        verifier = create_verifying_chain(llm, analysis_prompt, answer_extraction_prompt)
        samples_verified = []
        ...
        for sample in tqdm(samples, desc="Validating samples"):
            try:
                ...
                sources = self._text_corpus_db.get_by_ids([sample.answer_id_1, sample.answer_id_2])['documents']
                ...
                samples_verified.append(SampleTripletRAGChroma(
    ...
    def _raw_similarity_search_by_vector(self,
