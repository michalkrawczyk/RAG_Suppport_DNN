...
- suggestions (JSON formatted list from EXTRACT mode) or selected_terms (from ASSESS mode)
...
def create_domain_assessment_record(row: pd.Series) -> DomainAssessmentRecord:
    ...
    # Parse suggestions or selected_terms (JSON formatted)
    # Try 'suggestions' first (from EXTRACT mode), then 'selected_terms' (from ASSESS mode)
    suggestions_str = row.get("suggestions") or row.get("selected_terms", "[]")
    try:
        suggestions = (
            json.loads(suggestions_str) if isinstance(suggestions_str, str) else []
        )
    except json.JSONDecodeError as e:
        logger.warning(f"Failed to parse suggestions/selected_terms: {e}")
        suggestions = []
