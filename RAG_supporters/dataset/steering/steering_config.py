...
class SteeringConfig:
    ...
    def __post_init__(self):
        ...
        logging.info(
            f"SteeringConfig validated: mode={self.mode}, multi_label={self.multi_label_mode}"
        )

    @classmethod
    def from_single_mode(
        cls, mode: SteeringMode, multi_label_mode: str = "hard", random_seed: int = 42
    ) -> "SteeringConfig":
        """
        ...
        """
        if not isinstance(mode, SteeringMode):
            raise ValueError(f"mode must be SteeringMode instance, got {type(mode)}")
