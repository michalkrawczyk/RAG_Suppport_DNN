...
class ClusterParser:
    ...
    def __init__(
        ...
        if not self.clustering_json_path.exists():
            raise FileNotFoundError(
                f"Clustering JSON not found: {self.clustering_json_path}"
            )
        ...
        LOGGER.info(
            ...
            f"{'Yes' if self.keyword_embeddings else 'No'}"
        )
    ...
    def match_keyword_fuzzy(
        ...
        if not self.keyword_embeddings:
            LOGGER.debug(
                f"No exact match for '{keyword}' and no embeddings for fallback"
            )
        ...
        LOGGER.debug(
            f"Fuzzy matching for '{keyword}' requires embedding model "
            "(not available in parser)"
        )
        
        return (None, 0.0) if return_similarity else None
    
    def match_keywords_batch(
        self,
        keywords: List[str]
    ) -> List[Optional[int]]:
