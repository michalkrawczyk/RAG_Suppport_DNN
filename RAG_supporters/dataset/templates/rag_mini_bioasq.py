...
try:
    ...
    def OpenAIEmbeddings(*args, **kwargs):
        raise ImportError("OpenAIEmbeddings not found. Please install langchain_openai if you want to use OpenAIEmbeddings")
...
class RagMiniBioASQBase(BaseRAGDatasetGenerator):
    ...
    def __init__(self, dataset_dir: str, embed_function, **kwargs):
        ...
        self.load_dataset()
        self._embed_function = embed_function if embed_function is not None else OpenAIEmbeddings(
    ...
    def load_dataset(self):
        ...
        if len(self._text_corpus_db.get()['ids']) == 0:
            ...
            if not os.path.exists(os.path.join(self._dataset_dir, "passage_id_to_chroma_id.json")):
                raise FileNotFoundError("passage_id_to_chroma_id.json not found")

            # TODO: Check if necessery to store 'passage_id_to_chroma_id'
            with open(os.path.join(self._dataset_dir, "passage_id_to_chroma_id.json"), "r") as f:
    ...
    def validate_dataset(self):
        ...
        for i, doc_id in enumerate(question_docs['ids']):
            ...
            if not metadata.get("relevant_chroma_ids", None):
                raise ValueError(f"Question with ID {doc_id} does not have 'relevant_chroma_ids' metadata")
    ...
    def _init_text_corpus_db(self, batch_size: int = 10):
        ...
        for i, (passage, pid) in (
                enumerate(tqdm(zip(dataset["passage"], dataset["id"]), desc="Loading text corpus"))):
            metadata = {"id": pid}
        ...
        with open(os.path.join(self._dataset_dir, "passage_id_to_chroma_id.json"), "w") as f:
    ...
    def _init_questions_db(self, batch_size: int = 10):
        ...
        for i, (question, qid, relevant_ids) in (
                enumerate(tqdm(zip(dataset["question"], dataset["id"], dataset["relevant_passage_ids"]),
            ...
            metadata["relevant_chroma_ids"] = [self._passage_id_to_chroma_id[pid] for pid in relevant_ids]
    ...
    def _generate_positive_triplet_samples(self, question_embedding, relevant_passage_ids, **kwargs) \
        ...
        if len(relevant_passage_ids) > 1:
            ...
            for pid_1, pid_2 in relevant_combinations:
                sample_triplets.append(SampleTripletRAGChroma(
    ...
    def _generate_contrastive_triplet_samples(self, question_embedding, relevant_passage_ids,
                                              num_negative_samples: int = 5, keep_same_negatives=False, assume_relevant_best=True,**kwargs) \
            -> List[SampleTripletRAGChroma]:
        """
        ...
        possible_negatives = [pid for pid in self._passage_id_to_chroma_id.keys() if pid not in relevant_passage_ids]
        sample_triplets = []
    ...
    def _generate_similar_triplet_samples(self, question_embedding, relevant_passage_ids, score_threshold = 0.3,
        ...
        close_questions = self._raw_similarity_search_by_vector(question_embedding, k=kwargs.get("top_k", 3),
                                                                includes=["ids", "distances"])
        zipped_questions = zip(close_questions['ids'], close_questions['distances'])
        picked_questions = [qid for qid, dist in zipped_questions
