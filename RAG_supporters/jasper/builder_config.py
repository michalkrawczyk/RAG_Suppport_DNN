...
class BuildConfig:
    ...
    steering_probabilities: Dict[str, float] = field(
        default_factory=lambda: {"zero": 0.25, "centroid": 0.25, "keyword": 0.25, "residual": 0.25}
    )
    ...
    def __post_init__(self):
        ...
        if self.storage_format not in ("pt", "hdf5"):
            raise ValueError(f"storage_format must be 'pt' or 'hdf5', got {self.storage_format}")
    ...
    def update_post_build(
        self, n_pairs: int, n_questions: int, n_sources: int, n_keywords: int, n_clusters: int
    ) -> None:
