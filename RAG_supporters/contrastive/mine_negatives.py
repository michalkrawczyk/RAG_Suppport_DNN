...
class NegativeMiner:
    ...
    def _validate_inputs(
        ...
        if tier_proportions is not None:
            if not isinstance(tier_proportions, list):
                raise TypeError(
                    f"tier_proportions must be a list, got {type(tier_proportions)}"
                )
    ...
    def mine_all_negatives(self) -> Dict[str, torch.Tensor]:
        ...
        iterator = tqdm(range(self.n_pairs), desc="Mining negatives") if self.show_progress else range(self.n_pairs)
    ...
    def _mine_pair_negatives(
        ...
        tier4_negatives = self._sample_random(
            cluster_id, true_source_idx, self.tier_proportions[3]
        )
    ...
    def _sample_in_cluster(
        ...
        if n_to_sample < n_samples:
            additional = self.rng.choice(candidates, size=n_samples - n_to_sample, replace=True).tolist()
    ...
    def _validate_negatives(
        self, hard_negatives: torch.Tensor, negative_tiers: torch.Tensor
    ):
        """Validate mined negatives."""
        LOGGER.info("Validating mined negatives...")
        
        # Check shapes
        assert hard_negatives.shape == (self.n_pairs, self.n_neg), \
            f"hard_negatives shape mismatch: expected {(self.n_pairs, self.n_neg)}, got {hard_negatives.shape}"
        assert negative_tiers.shape == (self.n_pairs, self.n_neg), \
        ...
        assert hard_negatives.max() < self.n_sources, \
    ...
    def save(self, output_dir: Union[str, Path], results: Dict[str, torch.Tensor]):
        ...
        torch.save(
            ...
            output_dir / "hard_negatives.pt"
        )
        
        # Save tier labels
        torch.save(
            results['negative_tiers'],
            output_dir / "negative_tiers.pt"
        )
