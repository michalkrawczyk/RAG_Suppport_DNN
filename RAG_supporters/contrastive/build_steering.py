...
class SteeringBuilder:
    ...
    def build_centroid_steering(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        steering_vectors = torch.zeros(self.n_pairs, self.embedding_dim, dtype=torch.float32)
        ...
        for pair_idx in iterator:
            ...
            if len(keyword_ids) == 0 or self.keyword_embeddings is None:
                ...
                if norm > 1e-8:
                    ...
                    LOGGER.debug(f"Pair {pair_idx}: Keywords sum to zero, using fallback")
                    steering = self._get_fallback_steering(pair_idx)
    ...
    def build_residual_steering(self) -> torch.Tensor:
        ...
        for pair_idx in iterator:
            ...
            if self.normalize_residual:
                ...
                if norm > 1e-8:
                    ...
                    LOGGER.debug(f"Pair {pair_idx}: Question at centroid, zero residual")
        ...
        self._validate_steering_vectors(
            steering_vectors, "residual", allow_zero=not self.normalize_residual
        )
    ...
    def _validate_steering_vectors(
        self, steering: torch.Tensor, variant: str, allow_zero: bool = False
    ):
        ...
        if torch.isnan(steering).any():
            ...
            raise ValueError(f"{variant} steering contains {n_nan} NaN values")

        if torch.isinf(steering).any():
            n_inf = torch.isinf(steering).sum().item()
            raise ValueError(f"{variant} steering contains {n_inf} Inf values")
    ...
    def save(
        ...
        torch.save(steering_results["centroid"], output_dir / "steering_centroid.pt")
        torch.save(
            steering_results["keyword_weighted"], output_dir / "steering_keyword_weighted.pt"
        )
        torch.save(steering_results["residual"], output_dir / "steering_residual.pt")

        # Save distances
        torch.save(steering_results["distances"], output_dir / "centroid_distances.pt")
