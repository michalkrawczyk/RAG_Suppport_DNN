...
class TestCSVMergerNormalization:
    ...
    def test_normalize_minimal_columns(self):
        ...
        assert "relevance_score" in normalized.columns, "Should have 'relevance_score' column with 1.0"
    ...
    def test_normalize_all_columns(self):
        ...
        assert normalized["answer"].iloc[0] == "Python is a programming language", "Should preserve answer"
    ...
    def test_normalize_with_aliases(self):
        ...
        assert normalized["question"].iloc[0] == "What is Python?", "Should map 'question_text' to 'question'"
        assert normalized["source"].iloc[0] == "Python is a language", "Should map 'source_text' to 'source'"
        assert normalized["relevance_score"].iloc[0] == 0.8, "Should map 'score' to 'relevance_score'"
...
class TestCSVMergerKeywordParsing:
    ...
    def test_parse_keywords_comma_separated(self):
        ...
        assert result == ["python", "programming", "language"], "Should parse comma-separated keywords"
...
class TestCSVMergerDeduplication:
    ...
    def test_merge_duplicates_many_to_many_preserved(self):
        ...
        assert set(q1_pairs["source"]) == {"s1", "s2"}, "Question q1 should be paired with s1 and s2"
        ...
        assert set(s1_pairs["question"]) == {"q1", "q2"}, "Source s1 should be paired with q1 and q2"
...
class TestCSVMergerIDAssignment:
    ...
    def test_assign_ids_unique_questions(self):
        ...
        assert result.loc[result["question"] == "q1", "question_id"].nunique() == 1, "Same question should have same ID"
    ...
    def test_assign_ids_unique_sources(self):
        ...
        assert result.loc[result["source"] == "s1", "source_id"].nunique() == 1, "Same source should have same ID"
...
class TestCSVMergerIntegration:
    ...
    def test_merge_many_to_many_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert len(result) == 4, "Should preserve all 4 unique pairs in many-to-many relationship"
    ...
    def test_merge_with_exact_duplicates_in_many_to_many(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert python_lang_pair["relevance_score"].iloc[0] == 0.9, "Should keep max score from duplicates"
...
class TestCSVMergerInspectionMetadata:
    ...
    def test_create_inspection_metadata(self):
        ...
        assert metadata["metadata"]["clustering_source"] == "clusters.json", "Should reference clustering source"
