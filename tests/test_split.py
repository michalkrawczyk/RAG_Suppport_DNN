...
class TestDatasetSplitterInit:
    ...
    def test_init_valid(self, sample_data):
        ...
        assert splitter.n_pairs == sample_data["n_pairs"], "Should have correct number of pairs"
        assert (
            splitter.n_questions == sample_data["n_questions"]
        ), "Should have correct number of questions"
        assert splitter.train_ratio == 0.7, "Should have default train ratio"
        assert splitter.val_ratio == 0.15, "Should have default val ratio"
        assert splitter.test_ratio == 0.15, "Should have default test ratio"
    ...
    def test_init_custom_ratios(self, sample_data):
        ...
        assert splitter.train_ratio == 0.6, "Should have custom train ratio"
        assert splitter.val_ratio == 0.2, "Should have custom val ratio"
        assert splitter.test_ratio == 0.2, "Should have custom test ratio"
...
class TestDatasetSplitterSplit:
    ...
    def test_split_basic(self, sample_data):
        ...
        assert isinstance(results["train_idx"], torch.Tensor), "train_idx should be torch.Tensor"
        assert isinstance(results["val_idx"], torch.Tensor), "val_idx should be torch.Tensor"
        assert isinstance(results["test_idx"], torch.Tensor), "test_idx should be torch.Tensor"
    ...
    def test_split_covers_all_pairs(self, sample_data):
        ...
        assert (
            len(all_indices) == sample_data["n_pairs"]
    ...
    def test_split_no_question_leakage(self, sample_data):
        ...
        train_questions = set(sample_data["pair_indices"][results["train_idx"], 0].tolist())
        val_questions = set(sample_data["pair_indices"][results["val_idx"], 0].tolist())
        test_questions = set(sample_data["pair_indices"][results["test_idx"], 0].tolist())

        # Check no overlap
        assert (
            len(train_questions & val_questions) == 0
        ), "Questions should not leak between train and val"
        assert (
            len(train_questions & test_questions) == 0
        ), "Questions should not leak between train and test"
        assert (
            len(val_questions & test_questions) == 0
    ...
    def test_split_deterministic(self, sample_data):
        ...
        assert torch.equal(
            results1["train_idx"], results2["train_idx"]
        ), "Train split should be deterministic"
        assert torch.equal(
            results1["val_idx"], results2["val_idx"]
        ), "Val split should be deterministic"
        assert torch.equal(
            results1["test_idx"], results2["test_idx"]
    ...
    def test_split_small_dataset(self, small_data):
        ...
        assert len(val_questions & test_questions) == 0, "No question leakage between val and test"
    ...
    def test_split_single_cluster(self):
        ...
        splitter = DatasetSplitter(pair_indices=pair_indices, pair_cluster_ids=pair_cluster_ids)
...
class TestSplitDatasetFunction:
    ...
    def test_split_dataset_creates_files(self, sample_data):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Check files created
            assert (output_dir / "train_idx.pt").exists(), "Should create train_idx.pt"
            assert (output_dir / "val_idx.pt").exists(), "Should create val_idx.pt"
            assert (output_dir / "test_idx.pt").exists(), "Should create test_idx.pt"
    ...
    def test_split_dataset_custom_ratios(self, sample_data):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert train_ratio >= 0.7, f"Train ratio should be ~0.8, got {train_ratio:.3f}"
...
class TestEdgeCases:
    ...
    def test_unbalanced_cluster_distribution(self):
        ...
        pair_indices = torch.stack([torch.arange(100), torch.arange(100)], dim=1)
