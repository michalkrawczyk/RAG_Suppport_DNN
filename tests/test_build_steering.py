...
from RAG_supporters.dataset.build_steering import (
    SteeringBuilder,
    build_steering,
)
...
class TestSteeringBuilderInit:
    ...
    def test_init_valid(self, sample_embeddings):
        ...
        assert builder.n_pairs == sample_embeddings["n_pairs"], \
            "Should have correct number of pairs"
        assert builder.embedding_dim == sample_embeddings["dim"], \
            "Should have correct embedding dimension"
        assert builder.fallback_strategy == "centroid", \
            "Should have default fallback strategy"
    ...
    def test_init_custom_fallback(self, sample_embeddings):
        ...
        assert builder.fallback_strategy == "zero", \
...
class TestCentroidSteering:
    ...
    def test_centroid_steering_shape(self, sample_embeddings):
        ...
        assert steering.shape == (sample_embeddings["n_pairs"], sample_embeddings["dim"]), \
            "Steering vectors should have shape [n_pairs, dim]"
        assert distances.shape == (sample_embeddings["n_pairs"],), \
    ...
    def test_centroid_steering_normalized(self, sample_embeddings):
        ...
        assert torch.allclose(non_zero_norms, torch.ones_like(non_zero_norms), atol=1e-5), \
    ...
    def test_centroid_distances_range(self, sample_embeddings):
        ...
        assert (distances >= -1e-6).all(), \
            "Cosine distances should be >= 0"
        assert (distances <= 2.0 + 1e-6).all(), \
            "Cosine distances should be <= 2"
    ...
    def test_centroid_steering_no_nan_inf(self, sample_embeddings):
        ...
        assert not torch.isnan(steering).any(), \
            "Steering vectors should not contain NaN"
        assert not torch.isinf(steering).any(), \
            "Steering vectors should not contain Inf"
        assert not torch.isnan(distances).any(), \
            "Distances should not contain NaN"
        assert not torch.isinf(distances).any(), \
            "Distances should not contain Inf"
...
class TestResidualSteering:
    ...
    def test_residual_steering_unnormalized(self, sample_embeddings):
        ...
        assert non_unit.sum() > 0, \
            "Unnormalized residual steering should have non-unit vectors"
...
class TestBuildAllSteering:
    ...
    def test_build_all_steering_shapes(self, sample_embeddings):
        ...
        assert results["centroid"].shape == (n_pairs, dim), \
            "Centroid steering should have correct shape"
        assert results["keyword_weighted"].shape == (n_pairs, dim), \
            "Keyword steering should have correct shape"
        assert results["residual"].shape == (n_pairs, dim), \
            "Residual steering should have correct shape"
        assert results["distances"].shape == (n_pairs,), \
            "Distances should have correct shape"
...
class TestSteeringSave:
    ...
    def test_save_creates_files(self, sample_embeddings):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert (output_dir / "steering_centroid.pt").exists(), \
                "Should create centroid steering file"
            assert (output_dir / "steering_keyword_weighted.pt").exists(), \
                "Should create keyword steering file"
            assert (output_dir / "steering_residual.pt").exists(), \
                "Should create residual steering file"
            assert (output_dir / "centroid_distances.pt").exists(), \
                "Should create distances file"
    ...
    def test_save_with_precomputed_results(self, sample_embeddings):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert torch.allclose(loaded_centroid, results["centroid"]), \
...
class TestEdgeCases:
    ...
    def test_all_pairs_same_cluster(self, sample_embeddings):
        ...
        assert results["centroid"].shape[0] == sample_embeddings["n_pairs"], \
    ...
    def test_all_pairs_no_keywords(self, sample_embeddings):
        ...
        assert steering.shape[0] == sample_embeddings["n_pairs"], \
    ...
    def test_single_pair(self):
        ...
        assert results["centroid"].shape == (1, 64), \
            "Should handle single pair"
        assert results["distances"].shape == (1,), \
            "Should compute distance for single pair"
