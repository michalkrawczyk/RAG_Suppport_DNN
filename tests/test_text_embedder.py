...
def _mock_sentence_transformer(dim: int = 8):
    ...
    mock.encode.side_effect = lambda texts, **kw: np.random.rand(len(texts), dim).astype(np.float32)
    return mock
...
def _mock_langchain_embedder(dim: int = 8):
    ...
    mock = MagicMock()
    mock.embed_documents.side_effect = lambda texts: np.random.rand(len(texts), dim).tolist()
...
class TestTextEmbedderInit:
    ...
    def test_init_with_sentence_transformer(self):
        ...
        assert (
            embedder.model_type == "sentence-transformers"
