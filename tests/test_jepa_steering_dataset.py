...
from RAG_supporters.dataset import JEPASteeringDataset
...
def test_dataset_init(mock_dataset_dir):
    ...
    dataset = JEPASteeringDataset(mock_dataset_dir, split="train", epoch=0)
...
def test_dataset_splits(mock_dataset_dir):
    ...
    train_dataset = JEPASteeringDataset(mock_dataset_dir, split="train")
    val_dataset = JEPASteeringDataset(mock_dataset_dir, split="val")
    test_dataset = JEPASteeringDataset(mock_dataset_dir, split="test")
...
def test_getitem_schema(mock_dataset_dir):
    ...
    dataset = JEPASteeringDataset(mock_dataset_dir, split="train")
...
def test_determinism_with_same_epoch(mock_dataset_dir):
    ...
    dataset1 = JEPASteeringDataset(mock_dataset_dir, split="train", epoch=5)
    dataset2 = JEPASteeringDataset(mock_dataset_dir, split="train", epoch=5)
...
def test_different_epochs_different_samples(mock_dataset_dir):
    ...
    dataset1 = JEPASteeringDataset(mock_dataset_dir, split="train", epoch=0)
    dataset2 = JEPASteeringDataset(mock_dataset_dir, split="train", epoch=10)
...
def test_invalid_split_raises(mock_dataset_dir):
    ...
    with pytest.raises(ValueError, match="Split file not found"):
        JEPASteeringDataset(mock_dataset_dir, split="invalid")
...
def test_missing_config_raises():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        with pytest.raises(ValueError, match="Config file not found"):
            JEPASteeringDataset(tmpdir, split="train")
...
def test_missing_dataset_dir_raises():
    ...
    with pytest.raises(ValueError, match="Dataset directory not found"):
        JEPASteeringDataset("/nonexistent/path", split="train")
