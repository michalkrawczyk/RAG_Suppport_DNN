...
class TestTextAugmentationAgentInit:
    ...
    def test_init_with_custom_params(self):
        ...
        agent = TextAugmentationAgent(
            llm=mock_llm, verify_meaning=True, max_retries=5, batch_size=20
        )
...
class TestTextAugmentationAgentRephraseText:
    ...
    def test_rephrase_text_full_mode(self):
        ...
        result = agent.rephrase_text(
            text="Plants use photosynthesis to convert sunlight into energy.", mode="full"
        )
    ...
    def test_rephrase_text_sentence_mode(self):
        ...
        mock_llm = Mock(spec=BaseChatModel)
        mock_llm.invoke = Mock(return_value=AIMessage(content="Plants utilize photosynthesis."))

        agent = TextAugmentationAgent(llm=mock_llm)

        result = agent.rephrase_text(
            text="Plants use photosynthesis. It converts light to energy.", mode="sentence"
        )
    ...
    def test_rephrase_text_random_mode(self):
        ...
        mock_llm = Mock(spec=BaseChatModel)
        mock_llm.invoke = Mock(return_value=AIMessage(content="Rephrased text"))

        agent = TextAugmentationAgent(llm=mock_llm)

        result = agent.rephrase_text(text="Original text to rephrase.", mode="random")
    ...
    def test_rephrase_text_returns_none_on_error(self):
        ...
        result = agent.rephrase_text(text="Some text", mode="full")
...
class TestTextAugmentationAgentAugmentDataFrame:
    ...
    def test_augment_dataframe_questions_only(self):
        ...
        mock_llm = Mock(spec=BaseChatModel)
        mock_llm.invoke = Mock(return_value=AIMessage(content="Rephrased question?"))

        agent = TextAugmentationAgent(llm=mock_llm)

        df = pd.DataFrame({"question_text": ["Q1?", "Q2?"], "source_text": ["S1", "S2"]})

        result_df = agent.augment_dataframe(
            df, rephrase_question=True, rephrase_source=False, probability=1.0
        )
    ...
    def test_augment_dataframe_sources_only(self):
        ...
        mock_llm = Mock(spec=BaseChatModel)
        mock_llm.invoke = Mock(return_value=AIMessage(content="Rephrased source text."))
        ...
        result_df = agent.augment_dataframe(
            df, rephrase_question=False, rephrase_source=True, probability=1.0
        )
    ...
    def test_augment_dataframe_both(self):
        ...
        result_df = agent.augment_dataframe(
            df, rephrase_question=True, rephrase_source=True, probability=1.0
        )
    ...
    def test_augment_dataframe_with_probability(self):
        ...
        mock_llm = Mock(spec=BaseChatModel)
        mock_llm.invoke = Mock(return_value=AIMessage(content="Rephrased"))

        agent = TextAugmentationAgent(llm=mock_llm)

        df = pd.DataFrame({"question_text": ["Q1?", "Q2?"], "source_text": ["S1", "S2"]})

        result_df = agent.augment_dataframe(df, rephrase_question=True, probability=0.5)
    ...
    def test_augment_dataframe_custom_columns(self):
        ...
        df = pd.DataFrame({"query": ["Q1?", "Q2?"], "answer": ["A1", "A2"]})
...
class TestTextAugmentationAgentVerifyMeaning:
    ...
    def test_verify_meaning_preserved(self):
        ...
        result = agent.rephrase_text(text="Original text", mode="full")
...
class TestTextAugmentationAgentIntegration:
    ...
    def test_real_llm_integration(self):
        ...
        result = agent.rephrase_text(
            text="Plants convert sunlight into energy through photosynthesis.", mode="full"
        )
