...
def sample_data():
    ...
    pair_indices = torch.stack(
        [torch.randint(0, n_questions, (n_pairs,)), torch.randint(0, n_sources, (n_pairs,))], dim=1
    )
...
class TestNegativeMinerInit:
    ...
    def test_init_valid(self, sample_data):
        ...
        assert miner.n_pairs == sample_data["n_pairs"], "Should have correct number of pairs"
        assert miner.n_sources == sample_data["n_sources"], "Should have correct number of sources"
        assert (
            miner.n_clusters == sample_data["n_clusters"]
        ), "Should have correct number of clusters"
        assert miner.n_neg == 12, "Should have correct n_neg"
        assert miner.embedding_dim == sample_data["dim"], "Should have correct embedding dimension"
    ...
    def test_init_with_tier_proportions(self, sample_data):
        ...
        assert miner.tier_proportions == [3, 4, 3, 2], "Should have custom tier proportions"
    ...
    def test_init_auto_tier_proportions(self, sample_data):
        ...
        assert sum(miner.tier_proportions) == 12, "Auto tier proportions should sum to n_neg"
        assert len(miner.tier_proportions) == 4, "Should have 4 tiers"
    ...
    def test_init_cluster_structures_built(self, sample_data):
        ...
        assert hasattr(miner, "cluster_sources"), "Should have cluster_sources mapping"
        assert (
            len(miner.cluster_sources) == sample_data["n_clusters"]
        ...
        assert (
            total_sources == sample_data["n_sources"]
...
class TestNegativeMinerMining:
    ...
    def test_mine_all_negatives_shape(self, sample_data):
        ...
        assert "hard_negatives" in results, "Results should contain hard_negatives"
        assert "negative_tiers" in results, "Results should contain negative_tiers"

        assert results["hard_negatives"].shape == (
            sample_data["n_pairs"],
            12,
        ), f"hard_negatives shape should be {(sample_data['n_pairs'], 12)}"
        assert results["negative_tiers"].shape == (
            sample_data["n_pairs"],
            12,
    ...
    def test_mine_all_negatives_indices_valid(self, sample_data):
        ...
        assert hard_negatives.min() >= 0, "All negative indices should be >= 0"
        assert (
            hard_negatives.max() < sample_data["n_sources"]
    ...
    def test_mine_all_negatives_tiers_valid(self, sample_data):
        ...
        assert negative_tiers.min() >= 1, "All tier labels should be >= 1"
        assert negative_tiers.max() <= 4, "All tier labels should be <= 4"
    ...
    def test_mine_all_negatives_tier_distribution(self, sample_data):
        ...
        for tier_id, expected_count in enumerate(tier_proportions, start=1):
            ...
            assert (
                actual_count <= expected_total * 1.2
    ...
    def test_mine_single_pair(self, sample_data):
        ...
        assert results["negative_tiers"].shape == (1, 12), "Should have tier labels for single pair"
    ...
    def test_mine_with_small_n_neg(self, sample_data):
        ...
        assert results["hard_negatives"].shape[1] == 4, "Should mine 4 negatives per pair"
    ...
    def test_mine_with_large_n_neg(self, sample_data):
        ...
        assert results["hard_negatives"].shape[1] == 50, "Should mine 50 negatives per pair"
...
class TestNegativeMinerEdgeCases:
    ...
    def test_deterministic_with_seed(self, sample_data):
        ...
        assert torch.equal(
            results1["hard_negatives"], results2["hard_negatives"]
        ), "Results should be identical with same seed"
        assert torch.equal(
            results1["negative_tiers"], results2["negative_tiers"]
...
class TestNegativeMinerSaveLoad:
    ...
    def test_save(self, sample_data):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Check files exist
            assert (output_dir / "hard_negatives.pt").exists(), "Should save hard_negatives.pt"
            assert (output_dir / "negative_tiers.pt").exists(), "Should save negative_tiers.pt"
            ...
            assert torch.equal(
                loaded_negatives, results["hard_negatives"]
            ), "Loaded negatives should match original"
            assert torch.equal(
                loaded_tiers, results["negative_tiers"]
...
class TestMineNegativesFunction:
    ...
    def test_mine_negatives_function(self, sample_data):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Check results
            assert "hard_negatives" in results, "Should return hard_negatives"
            assert "negative_tiers" in results, "Should return negative_tiers"
