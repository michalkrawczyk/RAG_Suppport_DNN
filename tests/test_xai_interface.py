...
class TestExplainPrediction:
    ...
    def test_prediction_is_list(self):
        ...
        assert len(result["prediction"]) == D, \
            f"Prediction should have length {D} (embedding_dim)"
    ...
    def test_coarse_and_fine_none_for_jasper(self):
        ...
        assert result["fine_vector"] is None, \
            "fine_vector should be None for plain JASPERPredictor"
    ...
    def test_saves_to_file_when_save_path_given(self, tmp_path):
        ...
        if fig is not None:
            ...
            assert Path(save_path).exists(), \
                f"visualize_explanation should save plot to '{save_path}'"
...
class TestSaveXAIOutputs:
    def test_saves_valid_json(self, tmp_path):
        ...
        assert Path(out_path).exists(), \
            f"save_xai_outputs should create file at '{out_path}'"
        with open(out_path) as f:
            loaded = json.load(f)
        assert isinstance(loaded, list), \
            "save_xai_outputs should write a JSON array"
        assert len(loaded) == 1, \
            "Saved JSON should contain exactly 1 item"
    ...
    def test_saves_multiple_results(self, tmp_path):
        ...
        with open(out_path) as f:
            loaded = json.load(f)
        assert len(loaded) == 5, \
            "Saved JSON should contain 5 items for 5 XAI results"
